...
def get_paths(args, dataset, split):
    ...
    if (
        "dhcp_neo" in dataset and args["sample_as_mm"]
    ):  # required for fair validation between mono and multi-modal to sample only subjects with both modalities even for mono test
        args[dataset]["file_ids"] = [
            "*_desc-restore_T2w.nii.gz",
            "*_desc-restore_T1w.nii.gz",
            "*_desc-drawem9_dseg.nii.gz",
        ]
    all_paths = [
        sorted(
            [
                p
                for p in glob.glob(os.path.join(data_path, id))
                if any(sub in p for sub in subjects_split)
            ]
        )
    ...
    if "dhcp" in dataset:
        ...
        common_paths = [
            [
                p
                for p in paths
                if "_".join(os.path.basename(p).split("_")[:2]) in common_basenames
            ]
            for paths in all_paths
        ]
    elif dataset == "marsfet":
        common_sr = {
            path.split("/latest_nesvor/")[1].split("haste")[0] for path in all_paths[0]
        }
        common_seg = {
            path.split("/haste/")[1].split("reo-SVR")[0] for path in all_paths[1]
        }
        ...
        common_sr = {
            path.split("/default_reconst/")[1].split("_acq-haste")[0]
            for path in all_paths[0]
        }
        common_seg = {
            path.split("/combined/")[1].split(".nii.gz")[0] for path in all_paths[1]
        }
        common_basenames = set.intersection(common_sr, common_seg)
        common_paths = [
            [p for p in paths if any([common in p for common in common_basenames])]
    ...
    if (
        "dhcp_neo" in dataset
        and len(args["modalities"]["names"]) == 2
        and args["sample_as_mm"]
    ):  # required for fair validation between mono and multi-modal to sample only subjects with both modalities even for mono test
        args[dataset]["file_ids"] = [
            "*_desc-restore_T2w.nii.gz",
            "*_desc-drawem9_dseg.nii.gz",
...
def get_subject_paths(args, dataset, split, verbose=False):
    ...
    for i, path_mods in enumerate(paths_modalities):
        cons_met, conditions = check_constraints(
            args, path_mods[0], path_mods[-1], tsv_file, dataset=dataset
        )
        if cons_met:
            if verbose:
                print("Constraints met for ", path_mods[0])
            subjects_selected.append(
                {"modality_paths": path_mods, "conditions": conditions}
            )
        else:
            if verbose:
                print("Constraints not met for ", path_mods[0])
    if n_subjects > len(subjects_selected):
        if verbose:
            print(
                "Warning: n_subjects is larger than number of available subjects. "
...
def check_constraints(args, img_path, seg_path, tsv_file, dataset):
    ...
    if constraints["scan_age"]:
        constraints_sub["scan_age"] = get_attribute(
            img_path, tsv_file, dataset, attribute_name="scan_age"
        )
        if not constraints_met(args["constraints"], constraints_sub):
            return None, False
    if constraints["rad_score"]:
        constraints_sub["rad_score"] = get_attribute(
            img_path, tsv_file, dataset, attribute_name="rad_score"
        )
        if not constraints_met(args["constraints"], constraints_sub):
            return None, False
    if constraints["birth_age"] and "dhcp_neo" in dataset:
        constraints_sub["birth_age"] = get_attribute(
            img_path, tsv_file, dataset, attribute_name="birth_age"
        )
        if not constraints_met(args["constraints"], constraints_sub):
            return None, False
    if constraints["pnatal_age"] and "birth_age" in constraints_sub:
        constraints_sub["pnatal_age"] = (
            constraints_sub["scan_age"] - constraints_sub["birth_age"]
        )
        if not constraints_met(args["constraints"], constraints_sub):
            return None, False
    if constraints["sex"]:
        constraints_sub["sex"] = get_attribute(
            img_path, tsv_file, dataset, attribute_name="sex"
        )
        if not constraints_met(args["constraints"], constraints_sub):
            return None, False
    if constraints["brain_vol"]:
        constraints_sub["brain_vol"] = get_brain_volume(
            nib.load(seg_path), dataset_name=dataset
        )
        if not constraints_met(args["constraints"], constraints_sub):
            return None, False
    if constraints["label_vol"]:
        constraints_sub["label_vol"] = get_label_volume(
            nib.load(seg_path), labels=constraints["label_vol"]
        )
        if not constraints_met(args["constraints"], constraints_sub):
            return None, False
    if constraints["pathology"]:
        constraints_sub["pathology"] = get_attribute(
            img_path, tsv_file, dataset, attribute_name="pathology"
        )
        if not constraints_met(args["constraints"], constraints_sub):
            return None, False
...
def constraints_met(
    constraints, constraints_sub, verbose=False
):  # constraints is a subset of conditions
        ...
        try:
            if constraints[key] is not None:
                if key == "pathology":  # check if we want to sample this pathology
                    ...
                    constraints_sub[key] < constraints[key][0]
                    or constraints_sub[key] >= constraints[key][1]
                ):
                    return False
        except AttributeError:
            if verbose:
                print("No constraint for key ", key)
    return True
...
def assemble_subjects_v2(
    args, dataset, subjects, constraints, n_subs, start_idx, split
):
    # Convert list of dictionaries to a DataFrame for easy handling
    dict_converted = []
    for s in subjects:
        dict_converted.append(
            {
                "modality_paths": s["modality_paths"],
                "scan_age": torch.round(s["conditions"]["scan_age"])
                .to(torch.int)
                .item(),
        ...
        if "birth_age" in s["conditions"]:
            dict_converted[-1]["birth_age"] = (
                torch.round(s["conditions"]["birth_age"]).to(torch.int).item()
            )
        if "pnatal_age" in s["conditions"]:
            dict_converted[-1]["pnatal_age"] = (
                torch.round(s["conditions"]["pnatal_age"]).to(torch.int).item()
            )

    df = pd.DataFrame(dict_converted)
    sec_key = "birth_age" if "birth_age" in constraints else ""
    sampled = sample_uniformly(
        df,
        args[dataset]["n_subjects"][split],
        prim_key="scan_age",
        sec_key=sec_key,
...
def sample_uniformly(
    df, k, prim_key="scan_age", sec_key="birth_age", samp_strat="uniform"
):
        ...
        for scan_age in unique_scan_ages:
            ...
            while ns_group < num_samples_per_scan_age:
                ...
                if birth_age not in birth_age_groups.groups:
                    ...
                    if idx_break >= len(
                        b_a_prios
                    ):  # not enough samples in this scan_age group
                        break
                    continue
                idx_break = 0
                ...
                idx_prios += 1
                if (
                    idx_all_sampled >= len(birth_age_groups) * num_samples_per_scan_age
                ):  # all birth ages sampled
                    break
    else:
        while len(sampled_dicts) < min(
            (len(unique_scan_ages) * num_samples_per_scan_age), k
        ):
            for scan_age in unique_scan_ages:
                group = scan_age_groups.get_group(scan_age)
...
def assemble_subjects(args, subjects, dataset, start_idx, split):
    ...
    additional_cons = [
        key
        for key in args["conditions"].keys()
        if args["conditions"][key] and (key != "rad_score" and key != "state")
    ]
    ...
    if True:
        ...
        subs_sampled = assemble_subjects_v2(
            args, dataset, subjects, constraints, n_subs, start_idx, split
        )
        print(
            f"Sampled {len(subs_sampled)} of {args[dataset]['n_subjects'][split]} subjects."
        )
        args[dataset]["n_subjects"][split] = len(subs_sampled)
        for idx, sub in enumerate(subs_sampled):
            subjects_loaded.append(
                load_subject(args, sub, dataset, split, start_idx + idx)
            )
    else:
        subs_sampled = []
        # constraints = ['scan_age', 'birth_age']
        if args["sampling_strategy"] == "uniform_fillup":
            parent_bin = assemble_bin(
                args,
                dataset,
                subjects,
        ...
        for idx, i in enumerate(subs_sampled):
            subjects_loaded.append(
                load_subject(args, subjects[i], dataset, split, start_idx + idx)
            )
...
def load_subject(args, subject, dataset, split, idx, apply_mask=True):
    ...
    for j, mod_path in enumerate(reversed(subject["modality_paths"])):
        ...
        if apply_mask:
            ...
            modalities.append(
                torch.from_numpy(mod_np.squeeze() * mask).to(
                    args["device_dataset"], torch.float
                )
            )
        else:
            modalities.append(
                torch.from_numpy(mod_np.squeeze()).to(
                    args["device_dataset"], torch.float
                )
            )
        affines.append(
            torch.from_numpy(mod_nii.affine).to(args["device_dataset"], torch.float)
        )
    subject["modalities"] = modalities[::-1]  # reverse the reversed list
    subject["affines"] = affines[::-1]
    subject["modalities"][-1] = add_background_halo(
        args, subject["modalities"][-1], halo_width=1.5
    )
    subject["coords"], subject["values"] = coords_and_values(subject)
    subject["sub_id"], subject["ses_id"] = get_basename(
        subject["modality_paths"][0], dataset
    )
...
def coords_and_values(subject):
    ...
    values = torch.cat(
        [
            subject["modalities"][i][c_nz[:, 0], c_nz[:, 1], c_nz[:, 2]].flatten()[
                ..., None
            ]
            for i in range(len(subject["modalities"]))
        ],
        dim=-1,
    )
...
def add_background_halo(args, seg, halo_width=1.5):
    ...
    mask_bg = (ndi.gaussian_filter(mask_bg, sigma=halo_width) > 0.001).astype(
        np.uint8
    ) * bg_label
...
def determine_bounding_box(subjects):
    ...
    bbox_world = {
        "min": bbox_world[0],
        "max": bbox_world[1],
        "divisor": 1 / (bbox_world[1] - bbox_world[0]),
    }
...
def get_attribute(p_img, tsv_file, dataset_name, attribute_name):
    ...
    if dataset_name == "dhcp_fetal" or "dhcp_neo" in dataset_name:
        if attribute_name == "sex":
            ...
            attribute = tsv_file.loc[
                (tsv_file["subject_id"] == sub_id)
                & (tsv_file["session_id"] == int(ses_id[4:]))
            ][attribute_name].values[0]
    elif dataset_name == "feta":
        if attribute_name == "scan_age" or attribute_name == "rad_score":
            attribute = tsv_file.loc[(tsv_file["participant_id"] == sub_id)][
                attribute_name
            ].values[0]
        else:
            attribute = None
        ...
        if (
            attribute_name == "scan_age"
            or attribute_name == "pathology"
            or attribute_name == "rad_score"
        ):
            attribute = tsv_file.loc[
                (tsv_file["marsfet_subject_id"] == sub_id)
                & (tsv_file["marsfet_session_id"] == ses_id)
            ][attribute_name].values[0]
        if attribute_name == "sex":
            attribute = tsv_file.loc[
                (tsv_file["marsfet_subject_id"] == sub_id)
    ...
    if (
        ...
        and type(attribute) is not str
        and type(attribute) is not type(torch.tensor)
    ):
