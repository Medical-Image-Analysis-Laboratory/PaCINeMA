...
class Brains3D(Dataset):
        ...
        for dataset in self.args["datasets"]:
            self.subjects.extend(
                load_subjects(
                    self.args,
                    dataset=dataset,
                    start_idx=len(self.subjects),
                    split=self.split,
                )
            )
        self.bbox_vxl, self.bbox_world = (
            determine_bounding_box(self.subjects) if bbox is None else bbox
        )
        self.condition_bounds = self.normalize_conditions(
    ...
    def normalize_conditions(self, condition_bounds=None):
        if condition_bounds is None:
            ...
            cond_keys = ["scan_age"] + [
                key
                for key in self.subjects[0]["conditions"].keys()
            ...
            for cond in cond_keys:
                if cond == "rad_score":
                    ...
                    for i, label in enumerate(self.args["conditions"]["label_vol"]):
                        min_.append(
                            torch.tensor(
                                [sub["conditions"][cond][i] for sub in self.subjects]
                            ).min()
                        )
                        max_.append(
                            torch.tensor(
                    ...
                    max_ = torch.tensor(
                        [sub["conditions"][cond] for sub in self.subjects]
                    ).max()
                    min_ = torch.tensor(
                        [sub["conditions"][cond] for sub in self.subjects]
                    ).min()
        ...
        for cond in condition_bounds.keys():
            ...
            for sub in self.subjects:
                sub["conditions_normed"] = (
                    {} if "conditions_normed" not in sub else sub["conditions_normed"]
                )
                sub["conditions_normed"][cond] = (
                    (sub["conditions"][cond] - min_) / (max_ - min_)
                ) * 2 - 1
    ...
    def collate_fn(self, subjects):
        ...
        batch_val = (
            self.assemble_batch(subjects, holdout=True) if self.split == "val" else {}
        )
    ...
    def assemble_batch(self, subjects, holdout=False):
        ...
        for sub in subjects:
            ...
            for key in self.args["conditions"].keys():
                if self.args["conditions"][key]:
                    conds.append(
                        sub["conditions_normed"][key]
                        .view(1, -1)
                        .expand(sub["coords" + suffix].shape[0], -1)
                    )
            conds_batch.append(
                torch.cat(conds, dim=-1)
                if conds
                else torch.empty((sub["coords" + suffix].shape[0], 0)).to(torch.float)
            )
            idx = sub[f"idx_{self.split}"]
            idcs_batch.append(
                torch.ones_like(sub["coords" + suffix][:, 0]).to(torch.int64) * idx
            )
        coords = torch.cat(coords_batch, dim=0).to(torch.float)
        coords_normed = (
            (coords - self.bbox_vxl["min"]) * self.bbox_vxl["divisor"]
        ) * 2 - 1
        ...
        return {
            "coords_nrmd" + suffix: coords_normed[perm],
            "values" + suffix: values[perm],
            "conds_nrmd" + suffix: conds[perm],
            "idcs" + suffix: idcs[perm],
            "size" + suffix: coords.shape[0],
        }
    ...
    def get_conditions(self, key, normed=True):
        ...
        return torch.stack([sub[norm_key][key] for sub in self.subjects]).to(
            self.args["device"], torch.float
        )
    ...
    def plot_histogramm(self, key):
        ...
        if self.args["constraints"][key]:
            ...
            fig, ax = plt.subplots()
            bins = int(
                self.args["constraints"][key][1]
                - self.args["constraints"][key][0]
                + 0.5
            )
            bins = (
                np.arange(
                    self.args["constraints"][key][0],
                    self.args["constraints"][key][1] + 2,
                    1,
                )
                - 0.5
            )
            ...
            ax.hist(
                [
                    torch.round(sub["conditions"][key]).to(torch.int).numpy()
                    for sub in self.subjects
                ],
                bins=bins,
                edgecolor="black",
    ...
    def plot_volume_distribution(self, key, label=0):
        if self.args["constraints"][key]:
            label = 4
            res = 3
            scan_age_bins = np.arange(
                self.args["constraints"]["scan_age"][0],
                self.args["constraints"]["scan_age"][1] + 2,
                1,
            )
            birth_age_bins = np.arange(
                self.args["constraints"]["birth_age"][0],
                self.args["constraints"]["birth_age"][1] + 2,
                res,
            )
            ...
            for i, ba in enumerate(birth_age_bins):
                ...
                for sub in self.subjects:
                    bas = (
                        torch.round(sub["conditions"]["birth_age"])
                        .to(torch.int)
                        .numpy()
                    )
                    if bas <= ba and bas:
                        birth_age_volumes[ba][
                            torch.round(sub["conditions"]["scan_age"])
                            .to(torch.int)
                            .numpy()
                        ].append((sub["values"][:, -1] != label).sum())
                birth_age_volumes[ba] = {
                    key: (
                        np.mean(birth_age_volumes[ba][key])
                        if birth_age_volumes[ba][key]
                        else np.nan
                    )
                    for key in scan_age_bins
                }
