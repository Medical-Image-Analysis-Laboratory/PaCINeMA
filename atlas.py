...
class Atlas:
    def __init__(self, args):
        ...
        self.criterion = torch.compile(
            Criterion(args).to(args["device"]), disable=(not args["compile"])
        )
    ...
    def run(self):
        ...
        for epoch in range(self.args["epochs"]):
            ...
            if (epoch + 1) % self.args["val_every"] == 0:
                metrics = [
                    self.reconstruct_subject(
                        self.datasets["train"][0], epoch, split="train"
                    )
                ]
    ...
    def validate(self, epoch_train, split="val"):
        ...
        if self.args["generate_cond_atlas"]:
            self.generate_conditional_atlas()
        self._seed()
        ...
        if not self.args["val_init_by_mean"]:
            ...
            for epoch_val in range(self.args["val_epochs"]):
                if split == "val":
                    ...
                    loss_holdout = self.train(
                        self.dataloaders[split], scaler, split=split, holdout=True
                    )
                    if np.mean(loss_holdout["sr"]) < best_loss_holdout["sr"]:
                        best_loss_holdout["sr"] = np.mean(loss_holdout["sr"])
                        print(
                            f"New best SR loss: {np.mean(loss_holdout['sr'])} at epoch: {epoch_val}."
                        )
                    if np.mean(loss_holdout["seg"]) < best_loss_holdout["seg"]:
                        best_loss_holdout["seg"] = np.mean(loss_holdout["seg"])
                        print(
                            f"New best Seg loss: {np.mean(loss_holdout['seg'])} at epoch: {epoch_val}."
                        )
                    if self.args["logging"]:
                        wd.log({f"{split}/sr_loss_holdout": best_loss_holdout["sr"]})
                stime = time.time()
                if self.args["conditions"]["sex"]:
                    gt = (
                        self.datasets["val"]
                        .get_conditions(key="sex", normed=False)
                        .to(torch.int)
                    )
                    sex_pred_after = (
                        ((self.conditions["val"] >= 0.0) * 2 - 1)
                        .squeeze()
                        .to(torch.int)
                    )
                    sex_acc_mean = (
                        (sex_pred_after == gt).to(torch.float).mean().cpu().item()
                    )
                    # print mean and std
                    print(f"Accuracy sex prediction: {sex_acc_mean}")
                    if self.args["logging"]:
                        wd.log({f"{split}/sex_accuracy": sex_acc_mean})
                if self.args["conditions"]["birth_age"]:
                    gt = self.datasets["val"].get_conditions(
                        key="birth_age", normed=False
                    )
                    min_, max_ = (
                        self.datasets["val"].condition_bounds["birth_age"]["min"],
                        self.datasets["val"].condition_bounds["birth_age"]["max"],
                    )
                    pred = (
                        (self.conditions["val"] / self.args["cond_scale"] + 1) / 2
                    ) * (max_ - min_) + min_
                    ...
                    if self.args["logging"]:
                        wd.log({f"{split}/birth_age_mae": ba_mae_mean})
                # self.svr(key='scan_age')
                svr_acc, scan_age_acc_mean, pca_acc = self.svm_classifier(
                    key="scan_age"
                )

                loss_histories = self.train(
                    self.dataloaders[split], scaler, split=split
                )
                self.log_loss_history(
                    loss_histories, (epoch_train, epoch_val), stime, split=split
                )
        ...
        for idx in range(len(self.datasets[split])):
            metrics.append(
                self.reconstruct_subject(
                    self.datasets[split][idx],
                    (epoch_train, self.args["val_epochs"] - 1),
                    split=split,
                )
            )
    ...
    def train(self, dataloader, scaler=None, split="train", holdout=False):
        ...
        seg_weight = (
            self.args["seg_weight"]
            if split == "train"
            else float(self.args["seg_optim"])
        )
        ...
        for batch in dataloader:
            ...
            losses = self.train_inner(
                batch, sr_weight, seg_weight, n_smpls, split, scaler, holdout
            )
    ...
    def train_inner(
        self, batch, sr_weight, seg_weight, n_smpls, split, scaler, holdout=False
    ):
        losses = {
            "sr": [],
            "seg": [],
        ...
        for step in range(0, batch["size"], n_smpls):
            ...
            latents = torch.nn.functional.grid_sample(
                latents,
                coords_nrmd[:, None, None, None, :],
                align_corners=True,
                mode="bilinear",
                padding_mode="border",
            ).squeeze()
            conds_nrmd = (
                self.conditions[split]
                if len(self.conditions[split]) == 0
                else self.conditions[split][batch["idcs"][step : step + n_smpls]]
            )

            latents = torch.concat((latents, conds_nrmd), dim=-1)
            trafos = (
                self.trafos[split][batch["idcs"][step : step + n_smpls]]
                if self.args["trafo_dim"][split]
                else None
            )
            self.optim[split].zero_grad(set_to_none=True)
            with autocast(enabled=self.args["amp"]), torch.set_grad_enabled(
                not holdout
            ):
                output = self.models[split](coords_nrmd, latents, trafos)
                loss = self.criterion(
                    output, values, latents, trafos, sr_weight, seg_weight
                )
    ...
    def interpolate_subjects(self, subject1, subject2, ip_deg, split="train"):
        ...
        new_affine, shape = scale_affine(
            subject1["affines"][0].clone(),
            self.args["spacing_rec"],
        ...
        with torch.no_grad(), autocast(enabled=self.args["amp"]):
            trafos = (
                self.trafos[split][subject1[f"idx_{split}"], None].expand(
                    coords.shape[0], -1
                )
                if self.args["trafo_dim"][split]
                else None
            ...
            conds = (
                self.conditions[split][subject1[f"idx_{split}"]][None, ...].expand(
                    coords.shape[0], -1
                )
                if len(self.conditions[split]) > 0
                else self.conditions[split]
            )
            latents = torch.concat((latents, conds), dim=-1)
            modalities_rec = self.models[split].inference(
                coords, latents, img_shape=shape.tolist(), trafos=trafos
            )
            modalities_rec = modalities_rec[
                ..., : self.sr_dims + 1
            ]  # discard certainty maps for now
        del conds
        del latents
    ...
    def reconstruct_subject(self, subject, epoch, split="train"):
        ...
        with torch.no_grad(), autocast(enabled=self.args["amp"]):
            trafos = (
                self.trafos[split][subject[f"idx_{split}"], None].expand(
                    coords.shape[0], -1
                )
                if self.args["trafo_dim"][split]
                else None
            ...
            conds = (
                self.conditions[split][subject[f"idx_{split}"]][None, ...].expand(
                    coords.shape[0], -1
                )
                if len(self.conditions[split]) > 0
                else self.conditions[split]
    ...
    def get_condition_interval(self, key, normalize=False):
        min_, max_ = (
            self.datasets["val"].condition_bounds[key]["min"].to(self.args["device"]),
            self.datasets["val"].condition_bounds[key]["max"].to(self.args["device"]),
        )
        if len(min_.shape) == 0:
            interval = torch.linspace(min_, max_, self.args["cond_steps"]).to(
                self.args["device"]
            )
        else:
            interval = torch.stack(
                [
                    torch.linspace(min_[i], max_[i], self.args["cond_steps"]).to(
                        self.args["device"]
                    )
        ...
        if normalize:
            interval = (
                self.normalize(interval, (min_, max_), pre_fac=self.args["cond_scale"])
                .to(torch.float)
                .reshape(self.args["cond_steps"], -1)
            )
    ...
    def generate_conditional_atlas(self):
        ...
        atlases = []
        atlas_affine, atlas_shape = scale_affine(
            torch.eye(4), self.args["spacing_atlas"], self.datasets["train"].bbox_world
        )
        ...
        for cond_key, cond_flag in self.args["conditions"].items():
            if cond_flag:
                min_, max_, interval = self.get_condition_interval(
                    cond_key, normalize=True
                )
                # change the 3.1 to extrapoalte
                # TODO: pass the list pf condition combination pairs for a more restricted generation,, ptherwise combinaroial explosiion
                for cond_value in interval:
                    atlases.append(
                        self.generate_temporal_atlas(
                            cond_value * 3.1, coords, atlas_shape=atlas_shape, s_id=s_id
                        )
                    )
        if len(atlases) == 0:  # generate atlas without conditions
            for dname in self.args["datasets"]:
                atlases.append(
                    self.generate_temporal_atlas(
                        torch.empty((0,)), coords, atlas_shape=atlas_shape, c_l=dname
                    )
                )
    ...
    def generate_conditional_data(self):
        num_t = (
            self.args["constraints"]["scan_age"][1]
            - self.args["constraints"]["scan_age"][0]
        ) // self.args["age_step"]
        s_id = [
            (int(self.args["constraints"]["scan_age"][0]) + 1),  # start idx
            len(self.latents["train"]) // num_t,  # step size
        ]
        self.models["train"].eval()
        atlases = []
        atlas_affine, atlas_shape = scale_affine(
        ...
        for cond_key, cond_flag in self.args["conditions"].items():
            if cond_flag:
                ...
                for cond_value in interval:
                    atlases.append(
                        self.generate_temporal_atlas(
                            cond_value * 1.0, coords, atlas_shape=atlas_shape, s_id=s_id
                        )
                    )
        if len(atlases) == 0:  # generate atlas without conditions
            for dname in self.args["datasets"]:
    ...
    def generate_temporal_atlas(
        self, conditions, coords, atlas_shape, c_l=None, s_id=None
    ):
        temporal_atlas = []
        for age in range(
            int(self.args["constraints"]["scan_age"][0]) + 1,
            int(self.args["constraints"]["scan_age"][1]),
            self.args["age_step"],
        ):
            age_nrmd = self.normalize(
                age, self.args["constraints"]["scan_age"], pre_fac=1.0
            )
            with torch.no_grad(), autocast(enabled=self.args["amp"]):
                mean_latent = self.generate_mean_latent(
                    age_nrmd,
                    cond_key="scan_age",
                    cond_latents=c_l,
                ...
                if s_id is not None:
                    mean_latent = self.latents["train"][(age - s_id[0]) * s_id[1] - 1]
                mean_latent = mean_latent.expand(coords.shape[0], -1, -1, -1, -1)
                mean_latent = torch.nn.functional.grid_sample(
                    mean_latent,
                    coords[:, None, None, None, :],
                    align_corners=True,
                    mode="bilinear",
                ...
                mean_latent = torch.concat(
                    (
                        mean_latent,
                        conditions.to(mean_latent.device).expand(coords.shape[0], -1),
                    ),
                    dim=-1,
                )
                temporal_atlas.append(
                    self.models["train"]
                    .inference(coords, mean_latent, img_shape=atlas_shape.tolist())
                    .detach()
                    .cpu()
                )
                del mean_latent
    ...
    def generate_mean_latent(
        self, cond_value_nrmd, cond_key, cond_latents=None, std=1.0
    ):
        ...
        if cond_latents and not self.args["test_time"]:
            ...
            lat_idcs = (
                np.array([sub["dataset"] for sub in self.datasets["train"].subjects])
                == cond_latents
            )
        ...
        if cond_key == "scan_age":
            ...
            conds_nrmd = self.datasets["train"].get_conditions(
                key=cond_key, normed=True
            )[lat_idcs, None]

        latents = latents[:, : self.args["latent_dim"][0]]
        weights = torch.exp(-(((conds_nrmd - cond_value_nrmd) / std) ** 2))
        weights = weights / torch.sum(weights)
        weighted_mean_latent = torch.sum(
            latents * weights[:, :, None, None, None], dim=0, keepdim=True
        )
    ...
    def _init_attributes(self):
        ...
        self.conditions = {}
        self.args["cond_dims"] = sum(
            (
                1
                if isinstance(v, int) and v != 0
                else len([x for x in v if x != 0]) if isinstance(v, list) else 0
            )
            for v in self.args["conditions"].values()
        )
    ...
    def _init_optims(self, splits=("train", "val")):
        for split in splits:
            ...
            if self.args["trafo_dim"][split]:
                self.optim[split].add_param_group(
                    {
                        "name": f"trafos_{split}",
            ...
            if self.args["s_gamma"]:
                self.scheduler[split] = torch.optim.lr_scheduler.StepLR(
                    self.optim[split], step_size=1, gamma=self.args["s_gamma"][split]
                )
        ...
        for split in splits:
            if split == "val":
                if self.args["val_init_by_mean"]:
                    self.latents[split] = torch.empty(
                        (self.n_subjects[split], *self.args["latent_dim"])
                    ).to(self.args["device"])
                    for i, subject in enumerate(self.datasets[split]):
                        scan_age = self.normalize(
                            subject["conditions"]["scan_age"],
                            self.args["constraints"]["scan_age"],
                            pre_fac=1.0,
                        )
                    ...
                    self.latents[split] = torch.normal(
                        0, 0.01, size=(self.n_subjects[split], *self.args["latent_dim"])
                    ).to(self.args["device"])
                self.latents[split] = nn.Parameter(self.latents[split])
                self.latents_attrs[split] = {
                    "scan_age": self.datasets[split].get_conditions(
                        "scan_age", normed=False
                    ),
                    "scan_age_nrmd": self.datasets[split].get_conditions(
                        "scan_age", normed=True
                    ),
                    "dataset": np.array(
                        [sub["dataset"] for sub in self.datasets[split].subjects]
                    ),
                }
            elif not self.args[
                "test_time"
            ]:  # self.latents['train'].shape != (self.n_subjects[split], *self.args['latent_dim']):
                self.latents[split] = nn.Parameter(
                    torch.normal(
                        0, 0.01, size=(self.n_subjects[split], *self.args["latent_dim"])
                    ).to(self.args["device"])
                )
    ...
    def _init_trafos(self, splits=("train", "val")):
        for split in splits:
            ...
            if split == "train" and self.trafos["train"].shape == (
                self.n_subjects[split],
                self.args["trafo_dim"][split],
            ):
                continue
            self.trafos[split] = (
                nn.Parameter(
                    torch.zeros(
                        size=(self.n_subjects[split], self.args["trafo_dim"][split])
                    ).to(self.args["device"])
                )
                if self.args["trafo_dim"][split]
                else None
        ...
        for split in splits:
            ...
            for key in self.args["conditions"].keys():
                if self.args["conditions"][key]:
                    conds.append(
                        self.datasets[split]
                        .get_conditions(key, normed=True)
                        .view(self.n_subjects[split], -1)
                    )
            if len(conds) > 0:
                self.conditions[split] = (
                    torch.concat(conds, dim=-1) * self.args["cond_scale"]
                )
            else:
            ...
            if split == "val":
                ...
                if (
                    split == "val" and not self.args["val_init_by_mean"]
                ):  # in val, if alignment through latent optim., conds should be estimated, thus zero init. as param
                    # self.conditions[split] = nn.Parameter(0.0 * self.conditions[split])
                    self.conditions[split] = nn.Parameter(
                        torch.normal(
                            0,
                            0.01,  # self.args['cond_scale'],
                            size=self.conditions[split].shape,
                        ).to(self.args["device"])
                    )
    ...
    def _load_datasets(self, bbox=None, condition_bounds=None):
        ...
        stime = time.time()
        self.datasets = {
            "train": dataset.Brains3D(
                self.args, split="train", bbox=bbox, condition_bounds=condition_bounds
            )
        }
        bbox = (self.datasets["train"].bbox_vxl, self.datasets["train"].bbox_world)
        condition_bounds = self.datasets["train"].condition_bounds
        self.datasets["val"] = dataset.Brains3D(
            self.args, bbox, condition_bounds, split="val"
        )
        for split in ["train", "val"]:
            ...
            if n_samples < self.args["batch_size"][split]:
                ...
                print(
                    f"Batch size larger than num samples. Batch size for {split} set to {n_samples}"
                )

        self.dataloaders = {
            split: DataLoader(
                self.datasets[split],
                batch_size=self.args["batch_size"][split],
        ...
        self.n_subjects = {
            split: len(self.datasets[split]) for split in ["train", "val"]
        }
        print(
            "Datasets loaded. Subjects Train: {}, Val: {}. Time elapsed: {:.2f}".format(
                self.n_subjects["train"], self.n_subjects["val"], time.time() - stime
            )
        )
    ...
    def _init_models(self, splits=("train", "val")):
        ...
        if "train" in splits:
            ...
            self.models["train"] = torch.compile(
                self.models["train"], disable=not self.args["compile"]
            )
        if "val" in splits:
    ...
    def compute_metrics(self, subject, modalities_rec, new_affine, epoch, split):
        ...
        metrics = {
            ...
            "modalities_rec": modalities_rec,
            "new_aff": new_affine,
            "modalities_ref": subject["modalities"],
            "ref_aff": subject["affines"][0],
            "sub_id": f'{subject["sub_id"]}_{subject["ses_id"]}',
        }

        (
            metrics["modalities_rec"],
            metrics["new_aff"],
            metrics["psnr"],
            metrics["ssim"],
            metrics["ncc"],
        ...
        print(
            f"{split}, epoch {epoch}: Subject: {subject['sub_id']}_{subject['ses_id']}, "
    ...
    def log_loss_history(self, loss_histories, epoch, stime, split="train"):
        ...
        lat_magnitude = torch.mean(
            torch.linalg.vector_norm(self.latents[split], dim=-1)
        )
        trafo_loss = (
            torch.mean(self.trafos[split] ** 2) if self.trafos[split] is not None else 0
        )
        print(
            f"{split}: epoch {epoch}, loss_sr: {sr_mean:.4f}, loss_seg: {seg_mean:.4f}, lat_reg: {lat_reg_mean:.4f}, "
            f"loss_contrastive: {cont_mean:.4f}, lat_magnitude: {lat_magnitude:.4f}, "
            f"loss_trafo: {trafo_loss:.4f}, "
            f"loss_total: {total_mean:.4f}, time: {time.time() - stime:.2f}"
        )

        if self.args["logging"]:
            wd.log(
                {
                    "{}/loss_sr".format(split): torch.mean(
                        torch.tensor(loss_histories["sr"])
                    ),
                    "{}/loss_seg".format(split): torch.mean(
                        torch.tensor(loss_histories["seg"])
                    ),
                    "{}/loss_lat_reg".format(split): torch.mean(
                        torch.tensor(loss_histories["lat_reg"])
                    ),
                    "{}/loss_contrastive".format(split): torch.mean(
                        torch.tensor(loss_histories["contrastive"])
                    ),
                    "{}/lat_magnitude".format(split): lat_magnitude,
                    "{}/loss_trafo".format(split): trafo_loss,
                    "{}/loss_total".format(split): torch.mean(
                        torch.tensor(loss_histories["total"])
                    ),
                    "epoch": epoch,
                }
            )
    ...
    def log_validation(args, metrics_dict, epoch, split="train"):
        ...
        for i, mod in enumerate(args["modalities"]["names"][:-1]):
            metrics[f"psnr_{mod}"] = [
                torch.mean(torch.tensor([entry["psnr"][i] for entry in metrics_dict])),
                torch.std(torch.tensor([entry["psnr"][i] for entry in metrics_dict])),
            ]
            metrics[f"ssim_{mod}"] = [
                torch.mean(torch.tensor([entry["ssim"][i] for entry in metrics_dict])),
                torch.std(torch.tensor([entry["ssim"][i] for entry in metrics_dict])),
            ]
            metrics[f"ncc_{mod}"] = [
                torch.mean(torch.tensor([entry["ncc"][i] for entry in metrics_dict])),
                torch.std(torch.tensor([entry["ncc"][i] for entry in metrics_dict])),
            ]
            print(
                f"{split} validation images: "
        ...
        metrics["dice"] = (
            torch.mean(torch.stack([entry["dice"] for entry in metrics_dict]), dim=0),
            torch.std(torch.stack([entry["dice"] for entry in metrics_dict]), dim=0),
        )
        print(
            f"DICE: {metrics['dice'][0].tolist()}, DICE_STD: {metrics['dice'][1].tolist()}, "
        ...
        if args["logging"]:
            ...
            fig, axs = plt.subplots(
                len(metrics_dict), num_mods * 2, figsize=(15, 5 * len(metrics_dict))
            )
            ...
            for h, sub in enumerate(metrics_dict):
                for i in range(num_mods):
                    ...
                    vmax_rec = (
                        1 if i < len(sub["modalities_ref"]) - 1 else num_classes - 1
                    )
                    vmax_ref = (
                        mod_ref.max()
                        if i < len(sub["modalities_ref"]) - 1
                        else num_classes - 1
                    )
                    axs_flat[ax_counter].imshow(
                        mod_rec, cmap=cmap, vmin=0, vmax=vmax_rec
                    )
                    axs_flat[ax_counter + 1].imshow(
                        mod_ref, cmap=cmap, vmin=0, vmax=vmax_ref
                    )
                    axs_flat[ax_counter].set_title(
                        f'{args["modalities"]["names"][i]}_rec {sub["sub_id"]}'
                    )
                    axs_flat[ax_counter + 1].set_title(
                        f'{args["modalities"]["names"][i]}_ref {sub["sub_id"]}'
                    )
            ...
            wd.log(
                {
                    f"{split}/dice": metrics["dice"][0].mean(),
                    "epoch": epoch,
                    f"{split}/images": fig,
                }
            )
        ...
        if args["save_imgs"][split]:
            for h, sub in enumerate(metrics_dict):
                for i in range(len(sub["modalities_ref"])):
                    mod_rec, mod_ref = (
                        sub["modalities_rec"][..., i],
                        sub["modalities_ref"][i],
                    )
                    ...
                    save_img(
                        mod_rec.detach().cpu().numpy(),
                        aff_rec,
                        args["output_path"],
                        filename=name_rec,
                    )
                    save_img(
                        mod_ref.detach().cpu().numpy(),
                        aff_ref,
                        args["output_path"],
                        filename=name_ref,
                    )
    ...
    def save_model(self, epoch=None, target_metric=-9999):
        if self.args["save_model"]:
            state_dict = {
                "model_train": self.models["train"].state_dict(),
                ...
                "bbox": (
                    self.datasets["train"].bbox_vxl,
                    self.datasets["train"].bbox_world,
                ),
                "condition_bounds": self.datasets["train"].condition_bounds,
            }
            filename = (
                f"implicit_atlas_ep={epoch}.pt"
                if epoch is not None
                else "implicit_atlas_latest.pt"
            )
            ...
            if target_metric > self.args["best_target_metric"]:
                ...
                torch.save(
                    state_dict,
                    os.path.join(self.args["output_path"], "implicit_atlas_best.pt"),
                )
                print(
                    "Best model saved at: ",
                    os.path.join(self.args["output_path"], "implicit_atlas_best.pt"),
    ...
    def load_model(self, epoch=None):
        ...
        state_dict_train = {
            key.replace("_orig_mod.", ""): val for key, val in state_dict_train.items()
        }
    ...
    def svr(self, key="scan_age"):
        ...
        X_train = (
            ...
            .cpu()
            .numpy()
            .reshape(self.latents["train"].shape[0], -1)
        )
        ...
        X_val = (
            ...
            .cpu()
            .numpy()
            .reshape(self.latents["val"].shape[0], -1)
        )
        Y_val = (
            self.datasets["val"]
            .get_conditions(key, normed=False)
            .detach()
            .cpu()
            .numpy()
        )
    ...
    def svm_classifier(self, key="sex", val_ep=0):
        ...
        nca1 = make_pipeline(
            StandardScaler(),
            NeighborhoodComponentsAnalysis(n_components=1, random_state=42),
        )
        knn = KNeighborsClassifier(
            n_neighbors=min(
                min(len(self.latents["val"]), len(self.latents["train"])), 5
            )
        )
        ...
        tsne_results = tsne.fit_transform(
            np.concatenate((lats_train, lats_val), axis=0)
        )
        tsne_res_train = tsne_results[: len(lats_train)]
        tsne_res_val = tsne_results[len(lats_train) :]
        targets_train_nrmd = (
            self.latents_attrs["train"][key + "_nrmd"].cpu().numpy() + 1
        ) / 2
        targets_val_nrmd = (
            self.datasets["val"].get_conditions(key, normed=True).detach().cpu().numpy()
            + 1
        ) / 2
        ...
        if lats_train.shape[0] > num_classes:
            ...
            nca_acc = (
                np.sum(nca_predictions == gt_rounded) / len(gt_rounded)
                if key == "sex"
        ...
        ax.set_ylim(
            bottom=min(targets_rounded.min(), gt_rounded.min()) - 1,
            top=max(targets_rounded.max(), gt_rounded.max()) + 1,
        )
        ...
        if self.args["logging"]:
            wd.log(
                {
                    f"{key}_accuracy": svr_acc,
                    f"{key}_accuracy_nca": nca_acc,
                    f"{key}_accuracy_pca": pca_acc,
                }
            )
