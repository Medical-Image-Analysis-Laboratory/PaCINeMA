...
class CINA(nn.Module):
    def __init__(self, args):
        ...
        self.sr_net = Siren(
            args["in_dim"],
            args["latent_dim"][0] + args["cond_dims"],
            args["out_dim"],
            args["hidden_size"],
            args["num_hidden_layers"],
            f_om=args["first_omega"],
            h_om=args["hidden_omega"],
            outermost_linear=True,
            modulated_layers=args["modulated_layers"],
    ...
    def inference(self, coords, latent_vec, img_shape, trafos=None, step_size=50000):
        ...
        seg_hard = torch.argmax(output[..., self.sr_dims :], dim=-1).reshape(
            img_shape + [-1]
        )
        seg_soft = torch.nn.functional.softmax(
            output[..., self.sr_dims :], dim=-1
        ).reshape(img_shape + [-1])
    ...
    def connected_components(self, seg):
        ...
        # plt.show()
        mask_blur = (
            ndi.gaussian_filter(mask.astype(np.float32), sigma=1.0) > 0.001
        ).astype(np.uint8)
